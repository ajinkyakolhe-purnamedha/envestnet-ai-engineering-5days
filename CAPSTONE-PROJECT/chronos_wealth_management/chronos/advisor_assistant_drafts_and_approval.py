...
from chronos.advisor_analysis_reports_and_client_lists import (
    CONCENTRATION_THRESHOLD, HIGH_CASH_THRESHOLD, analyze_client_portfolio,
...
def build_note_draft_response(draft: AdvisorNoteDraft) -> AdvisorNoteDraftResponse:
    return AdvisorNoteDraftResponse(
        draft_id=draft.id, advisor_user_id=draft.advisor_user_id, client_user_id=draft.client_user_id,
        question=draft.question, note=draft.note, verdict=draft.verdict, note_source=draft.note_source,
        review_problems=json.loads(draft.review_problems_json), judge_verdict=draft.judge_verdict,
        status=draft.status, decision_reason=draft.decision_reason,
        created_simulated_date=draft.created_simulated_date,
    )


def submit_note_for_approval(db: Session, advisor_user_id: int, client_user_id: int, question: str, answer: AdvisorAssistantAnswerResponse, judge_verdict: str | None = None) -> AdvisorNoteDraftResponse:
    account = get_account_for_investor_user(db, client_user_id)
    draft = AdvisorNoteDraft(
        advisor_user_id=advisor_user_id, client_user_id=client_user_id, question=question,
        note=answer.note, verdict=answer.verdict, note_source=answer.note_source,
        review_problems_json=json.dumps(answer.review_problems), judge_verdict=judge_verdict,
        status=PENDING_STATUS, created_simulated_date=account.simulated_date,
    )
    db.add(draft)
    db.flush()
...
def decide_note_draft(db: Session, advisor_user_id: int, draft_id: int, decision: str, reason: str) -> AdvisorNoteDraftResponse:
...
def list_pending_drafts_for_advisor(db: Session, advisor_user_id: int) -> list[AdvisorNoteDraftResponse]:
    drafts = db.query(AdvisorNoteDraft).filter(
        AdvisorNoteDraft.advisor_user_id == advisor_user_id,
        AdvisorNoteDraft.status == PENDING_STATUS,
    ).order_by(AdvisorNoteDraft.id).all()
    return [build_note_draft_response(draft) for draft in drafts]


def list_approved_notes_for_client(db: Session, client_user_id: int) -> list[ClientAdvisorMessageResponse]:
    drafts = db.query(AdvisorNoteDraft).filter(
        AdvisorNoteDraft.client_user_id == client_user_id,
        AdvisorNoteDraft.status == APPROVED_STATUS,
    ).order_by(AdvisorNoteDraft.id).all()
    return [ClientAdvisorMessageResponse(
        draft_id=draft.id, note=draft.note, created_simulated_date=draft.created_simulated_date,
    ) for draft in drafts]


def answer_client_question_for_m9(db: Session, client_user_id: int, question: str, conversation_history: list[str] | None = None) -> AdvisorAssistantAnswerResponse:
    route = _route_reference_question(question)
    if route == TRADE_ROUTE:
        return AdvisorAssistantAnswerResponse(route=route, refused=True, verdict=None,
    ...
    if route == POLICY_ROUTE:
        return AdvisorAssistantAnswerResponse(route=route, refused=False, verdict=None,
            note=(f"Policy: single positions should stay at or below {CONCENTRATION_THRESHOLD:.0%}; "
    ...
    return AdvisorAssistantAnswerResponse(route=route, refused=False, verdict=verdict,
        note=(f"Portfolio value is ${metrics.total_value:,.2f}, with {metrics.cash_ratio:.0%} in cash and "
              f"{metrics.number_of_holdings} holdings. Verdict: {verdict}. {first_recommendation}"),
        note_source="m9_reference", review_problems=[], metrics=metrics)


def answer_with_memory(db: Session, client_user_id: int, question: str, conversation_history: list[str] | None = None) -> AdvisorAssistantAnswerResponse:
...
def condense_conversation_history(history: list[str], recent_turns_to_keep: int = 4) -> list[str]:
    if len(history) <= recent_turns_to_keep:
        return list(history)
    return ["Earlier questions: " + " | ".join(turn[:60] for turn in history[:-recent_turns_to_keep]), *history[-recent_turns_to_keep:]]
...
def _build_reference_verdict(metrics) -> str:
    if metrics.largest_position_ratio > CONCENTRATION_THRESHOLD:
        return f"outside guidelines: {metrics.largest_position_symbol} is {metrics.largest_position_ratio:.0%} of the portfolio (limit {CONCENTRATION_THRESHOLD:.0%})"
    if metrics.cash_ratio > HIGH_CASH_THRESHOLD:
        return f"outside guidelines: cash is {metrics.cash_ratio:.0%} of the portfolio (limit {HIGH_CASH_THRESHOLD:.0%})"
