...
def calculate_holding_gain_loss(shares: float, average_cost: float, current_price: float) -> float:
    market_value = calculate_holding_market_value(shares, current_price)
    cost_basis = calculate_holding_cost_basis(shares, average_cost)
    return calculate_unrealized_gain_loss(market_value, cost_basis)
...
def build_account_value_history(db: Session, account: Account) -> list[AccountValueHistoryPointResponse]:
    market_dates = list(
        db.scalars(
            select(Price.date)
            .where(
                Price.date >= STARTING_SIMULATED_DATE,
                Price.date <= account.simulated_date,
            )
            .distinct()
            .order_by(Price.date)
        )
    ...
    for market_date in market_dates:
        while next_trade_index < len(trades) and trades[next_trade_index].simulated_date <= market_date:
            ...
            shares_by_symbol[trade.symbol] = shares_by_symbol.get(
                trade.symbol, 0.0
...
def get_symbol_price_history(db: Session, symbol: str, end_date: date, trading_days: int = 60) -> list[Price]:
    ...
    return list(reversed(list(prices)))
